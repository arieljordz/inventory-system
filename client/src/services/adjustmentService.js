...
export const applyAdjustment = (payload) => {
  const res = api.post("/api/price-adjustments/apply", payload);
  return res.data;
};

// 📌 Get all adjustments with pagination & search
export const getAllAdjustments = (params = {}) => {
  const res = api.get("/api/price-adjustments", { params });
  return res.data;
};

// 📌 Get adjustments for a specific Product/Item (optional helper)
export const getAdjustmentsByTarget = async (targetType, targetId) => {
  const res = await api.get(`/api/price-adjustments/${targetType}/${targetId}`);
  return res.data;
};
